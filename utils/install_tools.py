        ...
        for tool, repo in GITHUB_REPOS.items():
            ...
            try:
                if go_present:
                    ...
                    cmd = (
                        f"go install -v github.com/{repo}/cmd/{tool}@latest"
                        if "projectdiscovery" in repo
                        else f"go install -v github.com/{repo}@latest"
                    )
                    if tool == "amass":
                    ...
                    if shutil.which(tool):
                        progress.update(
                            task_id,
                            description=f"[green]✓ {tool} installed (via go)",
                            completed=100,
                        )
                        continue
                ...
                if success:
                    progress.update(
                        task_id, description=f"[green]✓ {tool} installed", completed=100
                    )
                else:
                ...
                progress.update(
                    task_id, description=f"[red]✗ {tool} error: {str(e)}", completed=100
                )

    console.print("\n[bold green]✅ Tool installation process complete![/]")
    console.print(
        f"[yellow]💡 Ensure {TOOLS_DIR.absolute()} is in your PATH, or rely on internal wrappers.[/]\n"
    )
...
def _download_github_release(tool: str, repo: str, progress, task_id) -> bool:
    ...
    try:
        ...
        for asset in assets:
            ...
            if (
                "windows" in name
                and ("amd64" in name or "64-bit" in name)
